...
from .views import time_sheet_form_page_view , timesheet_page_view , manager_timesheet_page_view, client_timesheet_page_view, time_sheet_action_page_view, time_sheet_manager_action_page_view , client_payment_page_view
...
urlpatterns = [
    ...
    url(r'^clientpayment/(?P<employee_id>\d+)/(?P<project_id>\d+)/(?P<client_id>\d+)/(?P<day>\d+)/(?P<month>\d+)/(?P<year>\d+)/(?P<period>\d+)/$' , client_payment_page_view , name='clientpayment' ) ,
